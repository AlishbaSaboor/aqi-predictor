...
def predict(model, scaler, X, best_name):
...
def load_model():
    ...
    if best == "RandomForest":
        ...
        # Neural network was best but we use RF for serving
        model = joblib.load(os.path.join(MODELS_DIR, "random_forest.pkl"))
        best  = "RandomForest"
...
p24  = predict(model, scaler, X_in, best_name)

X_day2 = X_in.copy()
X_day2[0][FEATURES.index("aqi_lag_1")]       = p24
X_day2[0][FEATURES.index("aqi_lag_3")]       = p24
X_day2[0][FEATURES.index("aqi_lag_6")]       = p24
X_day2[0][FEATURES.index("aqi_lag_24")]      = float(latest.get("aqi", p24))
X_day2[0][FEATURES.index("aqi_lag_48")]      = float(latest.get("aqi_lag_24", p24))
X_day2[0][FEATURES.index("aqi_rolling_3h")]  = p24
X_day2[0][FEATURES.index("aqi_rolling_24h")] = (p24 + float(latest.get("aqi", p24))) / 2
X_day2[0][FEATURES.index("aqi_change_rate")] = p24 - float(latest.get("aqi", p24))
X_day2[0][FEATURES.index("hour")]            = float(latest.get("hour", 12))
X_day2[0][FEATURES.index("day")]             = (datetime.now() + timedelta(days=1)).day
X_day2[0][FEATURES.index("month")]           = (datetime.now() + timedelta(days=1)).month
X_day2[0][FEATURES.index("weekday")]         = (datetime.now() + timedelta(days=1)).weekday()
X_day2[0][FEATURES.index("is_weekend")]      = 1 if (datetime.now() + timedelta(days=1)).weekday() >= 5 else 0
p48 = predict(model, scaler, X_day2, best_name)

X_day3 = X_day2.copy()
X_day3[0][FEATURES.index("aqi_lag_1")]       = p48
X_day3[0][FEATURES.index("aqi_lag_3")]       = p48
X_day3[0][FEATURES.index("aqi_lag_6")]       = p48
X_day3[0][FEATURES.index("aqi_lag_24")]      = p24
X_day3[0][FEATURES.index("aqi_lag_48")]      = float(latest.get("aqi", p24))
X_day3[0][FEATURES.index("aqi_rolling_3h")]  = p48
X_day3[0][FEATURES.index("aqi_rolling_24h")] = (p48 + p24) / 2
X_day3[0][FEATURES.index("aqi_change_rate")] = p48 - p24
X_day3[0][FEATURES.index("day")]             = (datetime.now() + timedelta(days=2)).day
X_day3[0][FEATURES.index("month")]           = (datetime.now() + timedelta(days=2)).month
X_day3[0][FEATURES.index("weekday")]         = (datetime.now() + timedelta(days=2)).weekday()
X_day3[0][FEATURES.index("is_weekend")]      = 1 if (datetime.now() + timedelta(days=2)).weekday() >= 5 else 0
p72 = predict(model, scaler, X_day3, best_name)
...
for y_val, y_clr, name in [
    (50,  "#00c853", "Good"),
    (100, "#ffd600", "Moderate"),
    (150, "#ff6d00", "Unhealthy")
]:
...
st.caption(f"Best model: {best_name} | RMSE = {metrics[best_name]['rmse']} | R² = {metrics[best_name]['r2']}")
