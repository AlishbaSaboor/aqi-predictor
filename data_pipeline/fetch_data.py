...
def fetch_aqi_data(start_date=None, end_date=None):
    ...
    # Clamp end_date to today — APIs reject future dates
    if end_date > TODAY:
        end_date = TODAY

    print(f"📡 Fetching {start_date} → {end_date} ...")

    # ── Air quality (works for any historical date) ────────────
    ...
    # ── Weather — use archive for past, forecast for recent ────
    # archive API works for dates before ~5 days ago
    # forecast API works for recent/future dates
    five_days_ago = (datetime.now() - timedelta(days=5)).strftime("%Y-%m-%d")

    if end_date <= five_days_ago:
        # Pure historical — use archive API
        wx_url = "https://archive-api.open-meteo.com/v1/archive"
    else:
        # Recent or mixed — use forecast API
...
def store_in_mongodb(df):
    ...
    try:
        ...
        # Some duplicates rejected — count what was inserted
